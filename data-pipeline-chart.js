(function initializePipelineDashboard() {
  ...
  const pipelineView = document.getElementById("pipeline-feed-view");
  const tradingView = document.getElementById("tradingview-tools-view");
  const tradingViewFrame = document.getElementById("market-chart-frame");
  const elements = {
    ...
    tradingViewUpdated: document.getElementById("tradingview-updated"),
  };
  ...
  const requestedInstrumentId = query.get("instrument");
  ...
  const state = {
    ...
    viewOffset: 0,
    hoverIndex: null,
    activeInstrumentId: instrumentsById.has(requestedInstrumentId)
      ? requestedInstrumentId
      : instruments[0].instrumentId,
    activeTimeframe: TIMEFRAMES[requestedDefault] ? requestedDefault : "5m",
    activeView: requestedView === "tradingview" ? "tradingview" : "pipeline",
    historyLoaded: false,
    hasOlder: true,
    loading: false,
    loadingOlder: false,
  ...
  const updateBrowserUrl = () => {
    ...
    nextUrl.searchParams.set("instrument", state.activeInstrumentId);
    nextUrl.searchParams.set("view", state.activeView);
    nextUrl.searchParams.set("build", "10");
    nextUrl.hash = "market-dashboard";
  ...
  const tradingViewUrl = () => {
    ...
    const params = new URLSearchParams({
      ...
      symbol: providerSymbols[instrument.instrumentId] || instrument.instrumentId,
      interval: "5",
      hidesidetoolbar: "0",
      symboledit: "0",
      saveimage: "1",
  ...
  const loadTradingView = ({ force = false } = {}) => {
    if (!tradingViewFrame) return;
    const instrument = activeInstrument();
    const loadKey = `${instrument.instrumentId}:${document.documentElement.dataset.theme || "dark"}`;
    if (!force && tradingViewLoadedFor === loadKey) return;
    tradingViewLoadedFor = loadKey;
    if (elements.tradingViewStatus) {
      elements.tradingViewStatus.textContent = `Loading ${instrument.symbol} provider chart`;
    }
    if (elements.tradingViewUpdated) {
      elements.tradingViewUpdated.textContent = `v10 · requested ${providerSymbols[instrument.instrumentId] || instrument.instrumentId}`;
    }
    tradingViewFrame.title = `${instrument.symbolName} TradingView chart`;
    tradingViewFrame.src = tradingViewUrl();
  };

  const setChartView = (view) => {
    state.activeView = view === "tradingview" ? "tradingview" : "pipeline";
    if (pipelineView) pipelineView.hidden = state.activeView !== "pipeline";
    if (tradingView) tradingView.hidden = state.activeView !== "tradingview";
    viewButtons.forEach((button) => {
      const selected = button.dataset.chartView === state.activeView;
      button.classList.toggle("is-active", selected);
      button.setAttribute("aria-selected", String(selected));
      button.setAttribute("tabindex", selected ? "0" : "-1");
    });
    if (state.activeView === "tradingview") loadTradingView();
  ...
  if (tradingViewFrame) {
    tradingViewFrame.addEventListener("load", () => {
      const instrument = activeInstrument();
      if (elements.tradingViewStatus) {
        elements.tradingViewStatus.textContent = `${instrument.symbol} provider chart ready`;
      }
      if (elements.tradingViewUpdated) {
        elements.tradingViewUpdated.textContent = `v10 · ${providerSymbols[instrument.instrumentId] || instrument.instrumentId} · TradingView`;
      }
    });
  }
